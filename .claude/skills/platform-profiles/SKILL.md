...
```bash
# Detect from CI config files
[ -f ".gitlab-ci.yml" ] && PLATFORM="gitlab"
[ -f "bitbucket-pipelines.yml" ] && PLATFORM="bitbucket"
[ -d ".github" ] && PLATFORM="github"

# Fallback: detect from git remote URL
PLATFORM=$(git remote get-url origin 2>/dev/null | grep -oE '(github|gitlab|bitbucket)' | head -1)
```
...
| `<figure>`/`<figcaption>` | Rendered | Rendered | **No** — use `<p>` + `<em>` pattern |
| Mermaid diagrams | Yes | Yes (+ PlantUML) | **No** — use pre-rendered SVG |
| Math/LaTeX | Yes | Yes | **No** |
| Task lists `- [ ]` | Yes | Yes | **No** — renders as plain text |
| Footnotes | Yes | Yes (native) | **No** |
...
1. **Callouts** — Replace `> [!NOTE]` with `**Note:**` bold inline (already recommended in `doc-standards` for cross-renderer)
